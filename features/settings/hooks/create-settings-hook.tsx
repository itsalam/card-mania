...
export function createSettingsHooks<R extends SettingsRegistry>(
  ...
  function useSetting<K extends keyof R>(key: K) {
    ...
    const value = useSettingsStore((s: SettingsStoreState) =>
      s.getEffective(String(key))
    ) as InferSettingValue<R, K>
    ...
    const set = async (v: InferSettingValue<R, K>) => {
      ...
      const writes = Promise.all([
        setTier(String(key), 'local', v),
        setTier(String(key), 'remote', v),
      ])
      return writes.then(() => {
        return { prevVal: value, curr: v }
      }) //TODO: Debug outputs
    }
