...
export const suggestedVariantsOptions = (args: {
  maxResults?: number
  search?: string
  cardId: string
}) =>
  queryOptions({
    queryKey: ['suggested-variants', args.maxResults ?? 8, (args.search ?? '').trim()],
    // TanStack v5 will pass { signal } here if the adapter supports it
    queryFn: async (): Promise<Partial<Variant> & Pick<Variant, 'id' | 'name'>[]> => {
      const { data, error } = await getSupabase()
        .rpc('most_used_variants', {
