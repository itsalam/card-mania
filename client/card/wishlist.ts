...
export function useIsWishlisted(kind: ItemKinds, ids: string[]) {
  ...
  const query = useQuery({
    ...
    queryKey,
    queryFn: async () => {
      const result = await wishlistBatcher.request(kind, [...unvisited.values()])
      ...
      qc.setQueryData<Set<string>>(negativeQueryKey, (prev) => union(negatives, toSet(prev)))
      return newPos
    },
    staleTime: 60_000,
  })
...
type WishlistRow = Database['public']['Functions']['collection_item_query']['Returns'][number]
