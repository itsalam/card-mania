...
async function fetchCompaniesWithGrades(companySlug?: string): Promise<Graders[]> {
  ...
  return payload.map((c) => ({
    ...
    slug: c.slug,
    name: c.name,
    grades: [...c.grade_conditions].sort((a, b) => b.grade_value - a.grade_value),
  }))
}
...
export function useGradingConditions(opts?: { companySlug?: string }) {
  ...
  return useQuery({
    queryKey: slug ? gradingConditionsKeys.byCompany(slug) : gradingConditionsKeys.all,
