...
type UseCardQueryOpts = {
  ...
  queryOptions?: Parameters<
    typeof useQuery<TCardRes & { status?: number }, Error, TCard | undefined>
...
export function useCardQuery(
  ...
  const { entry, setCard } = useCardStore((s) => ({
  ...
  const query = useQuery<TCardRes & { status?: number }, Error, TCard | undefined>({
    ...
    queryFn: async () => {
      ...
      const { data, response } = await invokeFx<typeof payload, TCardRes>('fetch-card', payload, {
        parseOut: CardResponse,
        useQueryParams: true,
      })
