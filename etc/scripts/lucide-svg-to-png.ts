...
function parseArgs() {
  ...
  if (i === -1 || !process.argv[i + 1]) {
    console.error('Usage: tsx etc/scripts/lucide-svg-to-png.ts --config config/icons.json')
    process.exit(1)
  }
...
async function buildPngs(cfg: Config) {
  ...
  for (const name of cfg.icons) {
    const svg = await fs.readFile(svgPathForIcon(cfg.package!, name, cfg.prefix))
    const one = cfg.baseSize!
    await sharp(svg)
      .resize(one)
      .png()
      .toFile(path.join(assetDir, `${name}.png`))
    await sharp(svg)
      .resize(one * 2)
      .png()
      .toFile(path.join(assetDir, `${name}@2x.png`))
    await sharp(svg)
      .resize(one * 3)
      .png()
      .toFile(path.join(assetDir, `${name}@3x.png`))
  }
  return assetDir
}
...
async function main() {
  ...
  console.log(`✅ Exported ${cfg.icons.length} icons → ${path.resolve(cfg.outDir!)}`)
