...
export default function useFieldState({
  ...
  useEffect(() => {
    if (Constants.isWeb && !_value && defaultValue && defaultValue !== value) {
  ...
  const validateField = useCallback(
    (valueToValidate = value) => {
      const [_isValid, _failingValidatorIndex] = validate(valueToValidate, validateProp)
      setIsValid(_isValid)
  ...
  const onChangeText = useCallback<NonNullable<TextInputProps['onChangeText']>>(
    ...
    [_onChangeText, validateOnChange, debouncedValidateField, validateField]
  )
  ...
  const fieldState = useMemo(() => {
    return {
      ...
      isValid: validationMessage && !validateProp ? false : (isValid ?? true),
      isFocused,
...
export const useAnimatedColors = (ctxColor: string, ctxOpacity: number) => {
  ...
  const opacity = useDerivedValue(() => interpolate(ctxOpacityValue.value, [0, 1], [0.4, 1]))

  const color = useDerivedValue(() => {
    const c = interpolateColor(colorT.value, [0, 1], [fromColor.value!, toColor.value!, 'RGB'])
