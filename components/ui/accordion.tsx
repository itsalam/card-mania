...
const Trigger = Platform.OS === 'web' ? View : TouchableOpacity
...
function AccordionTrigger({
  ...
  return (
    <TextClassContext.Provider
      value={cn('text-left font-medium', Platform.select({ web: 'group-hover:underline' }))}
    >
  ...
  return (
    <TextClassContext.Provider value="">
      <AccordionPrimitive.Content
        ...
        {...props}
      >
        <Animated.View className={cn('pb-4', className)}>{children}</Animated.View>
      </AccordionPrimitive.Content>
    </TextClassContext.Provider>
  )
}
