...
function DropdownMenuSubContent({
  ...
  return (
    <DropdownMenuPrimitive.SubContent
      className={cn(
        'z-dropdownMenu min-w-[8rem] overflow-hidden rounded-md border border-border mt-1 bg-popover p-1 shadow-md shadow-foreground/5 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2',
        open
    ...
    <DropdownMenuPrimitive.Portal hostName={portalHost}>
      ...
      >
        <DropdownMenuPrimitive.Content
          className={cn(
            'z-dropdownMenu min-w-[8rem] overflow-hidden rounded-md border border-border bg-popover p-1 shadow-md shadow-foreground/5 web:data-[side=bottom]:slide-in-from-top-2 web:data-[side=left]:slide-in-from-right-2 web:data-[side=right]:slide-in-from-left-2 web:data-[side=top]:slide-in-from-bottom-2',
            open
              ? 'web:animate-in web:fade-in-0 web:zoom-in-95'
...
function DropdownMenuItem({
  ...
  className?: string
  inset?: boolean
  onClosePress?: boolean
}) {
  return (
    <TextClassContext.Provider value="select-none native:text-lg text-popover-foreground web:group-focus:text-accent-foreground">
  ...
  return (
    ...
    >
      <View className="absolute left-2 flex h-3.5 w-3.5 items-center justify-center">
        <DropdownMenuPrimitive.ItemIndicator className="border-1 border-foreground rounded-sm">
...
function DropdownMenuLabelText({ className, ...props }: TextProps) {
  return (
    <Text
      className={cn(
        'text-sm native:text-base font-semibold text-foreground web:cursor-default',
        className
      )}
      {...props}
    />
