...
import { findNodeHandle, LayoutChangeEvent, NativeMethods, ViewProps } from 'react-native'
...
export function useMeasure<T extends HasMeasure>(opts: Options = {}) {
  const { round = false, onLayout: externalOnLayout, layoutOnce = true, onMeasure } = opts
  ...
  const measureInWindow = useCallback(() => {
    ...
    ref.current?.measure((_x, _y, w, h, px, py) => {
      setLayout((prev) => {
        ...
        const height = round ? Math.round(base.height) : base.height
