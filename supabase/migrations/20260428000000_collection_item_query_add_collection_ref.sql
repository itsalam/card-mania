-- Add collection_ref to collection_item_query so the client can group selling-collection
-- items by their originating storefront collection. The column already exists on
-- collection_items but was never included in the function's RETURNS TABLE or SELECT.
--
-- DROP is required because CREATE OR REPLACE cannot change a function's return type.
-- Drop both the 4-param and 5-param overloads that may exist across migrations.

drop function if exists public.collection_item_query(uuid, timestamptz, text, int) cascade;
