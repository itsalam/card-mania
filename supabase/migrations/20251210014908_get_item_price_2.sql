-- Recreate collection_item_query ensuring uniqueness per collection_item id
...
set search_path = public as $$
  with rows as (
    ...
    left join public.cards c on ci.item_kind = 'card'::public.item_kind and ci.ref_id = c.id
    ...
    where ci.collection_id = p_collection_id
      and (p_page_param is null or ci.created_at < p_page_param)
      and (
        p_search is null
        or p_search = ''
        or (ci.item_kind = 'card'::public.item_kind and c.name ilike ('%' || p_search || '%'))
      )
  ...
  from rows r
  order by r.created_at desc, r.collection_item_id;
$$;
