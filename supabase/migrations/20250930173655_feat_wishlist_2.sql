...
create or replace view public.wishlist_cards_enriched as
...
alter table public.wishlist_totals enable row level security;

create policy "read own total"
on public.wishlist_totals
for select
to authenticated
...
stable
as $$
  -- For 'card', sum the selected grades; missing/null = 0
  select case
    when p_kind = 'card' then (
      ...
      from public.cards c,
           lateral (
             select jsonb_array_elements_text(coalesce(p_metadata->'grades','[]'::jsonb)) as g
           ) sel
      where c.id = p_ref_id
    )
    else 0
...
after insert or update of kind, ref_id, metadata or delete
on public.wishlist
...
begin
  ...
  if TG_OP = 'UPDATE' and (NEW.grades_prices is distinct from OLD.grades_prices) then
    -- For this one card, compute per-user delta and bump totals.
    -- Delta for a row = sum( NEW[g] - OLD[g] ) over selected grades.
    update public.wishlist_totals t
    set total_cents = t.total_cents + sub.delta_cents,
        computed_at = now()
    from (
      select w.user_id,
             coalesce(sum(
               coalesce( (NEW.grades_prices->>g)::int, 0 )
             - coalesce( (OLD.grades_prices->>g)::int, 0 )
             ), 0) as delta_cents
      from public.wishlist w,
    ...
    insert into public.wishlist_totals(user_id, total_cents, computed_at)
    select sub.user_id, sub.delta_cents, now()
    from (
      select w.user_id,
             coalesce(sum(
               coalesce( (NEW.grades_prices->>g)::int, 0 )
...
-- simple compute
create or replace function public.wishlist_total()
returns int
language sql
stable
as $$
  select coalesce((
    select total_cents from public.wishlist_totals
...
create or replace function public.wishlist_recompute_total()
returns int
language sql
security definer
...
as $$
  ...
  total as (
    select coalesce(sum(row_cost),0)::int as t from rows
  ),
  up as (
    insert into public.wishlist_totals(user_id, total_cents, computed_at)
    select auth.uid(), t, now() from total
    on conflict (user_id) do update
      set total_cents = excluded.total_cents,
          computed_at = excluded.computed_at
    returning total_cents
  )
  select total_cents from up;
$$;
