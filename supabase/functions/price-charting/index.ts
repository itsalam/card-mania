...
const COOL_OFF_MS = Number(
  Deno.env.get("IMAGE_COOL_OFF_MS") ?? `${24 * 60 * 60 * 1000}`,
);
const PRICECHARTING_CACHE_TTL = Number(
  Deno.env.get("PRICECHARTING_CACHE_TTL") ?? "300",
);
...
const convertPCEntrytoSearchItem = async (
  p: PriceChartingEntry,
...
Deno.serve(async (req) => {
  ...
  const cacheUpsert =
    (url.searchParams.get("skipCacheUpsert") ?? "false") === "false";
  ...
  if (!blendedCacheHit) {
    ...
    if (!vendorCacheHit) {
      providerCachePromise = fetchFromProvider(
        pcBase,
        pcKey,
        id ?? undefined,
        q,
    ...
    const rpcBlendedPromise = fetch(
      `${supa}/rest/v1/rpc/search_cards_blended`,
      {
        method: "POST",
        headers: {
    ...
    ).then((rpc) => rpc.json() as Promise<Array<BlendedSearchResultItem>>).then(
      (json) => {
        localRows = json;
      },
    );
  ...
  if (cacheUpsert) {
    if (!vendorCacheHit) {
      provider_query_key = provider_query_key ??
        await sha256HexStr(`${provider}:${queryHash}`);
...
async function getCandidateTopUrlOrPrewarm(
  card: Card,
  supa: string,
  srole: string,
) {
...
async function fetchInternalIds(
  ...
  if (vendorIds.length) {
    ...
    const rows = await resp.json() as Array<
      { external_id: string; card_id: string | null }
    >;
...
async function isHot(
  ...
  return row.hits >= HOT_THRESHOLD &&
    (!row.committed_at ||
      (Date.now() - new Date(row.committed_at).getTime()) > COOL_OFF_MS);
}

const fetchBlendedCache = async (
  supa: string,
  srole: string,
  queryHash: string,
) => {
  ...
  const row = rows?.[0];
  const fresh = row &&
    (new Date(row.updated_at).getTime() + row.ttl_seconds * 1000) > Date.now();
...
const fetchProviderCache = async (
  supa: string,
  srole: string,
  queryHash: string,
) => {
  ...
  const cacheUrl =
    `${supa}/rest/v1/provider_search_cache?provider_query_key=eq.${
      encodeURIComponent(provider_query_key)
...
const fetchFromProvider = async (
  ...
  const priceEntries: PriceChartingEntry[] = id
    ...
    : raw.products.slice(0, parseInt(limit ?? "20")).map((
...
const upsertProviderCache = (
  ...
  EdgeRuntime.waitUntil(fetch(`${supa}/rest/v1/provider_search_cache`, {
    method: "POST",
    headers: {
      "content-type": "application/json",
      "apikey": srole,
    ...
    body: JSON.stringify({
      ...
      expires_at: new Date(Date.now() + PRICECHARTING_CACHE_TTL * 1000)
        .toISOString(),
    }),
  }));
};
...
const upsertBlendedCache = (
  ...
  EdgeRuntime.waitUntil(fetch(`${supa}/rest/v1/search_cache`, {
    method: "POST",
    headers: {
      "content-type": "application/json",
      "apikey": srole,
