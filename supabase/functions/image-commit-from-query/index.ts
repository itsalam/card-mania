...
Deno.serve(async (req) => {
  try {
    ...
    if (typeof query_hash === 'string' && query_hash) {
      const cacheCommits = await commitCacheFromQueryHash(query_hash, sample)
      ...
      const promises = await Promise.all([cardImagePromise, commitUpdatedPromise])
    ...
    if (Array.isArray(body?.hashes) && body.hashes.length) {
      ...
      return new Response(JSON.stringify({ ok: true, mode: 'batch', count: hashes.length }), {
        status: 202,
        headers: { 'content-type': 'application/json' },
      })
    }
    ...
    const { data: hotRows, error: hotErr } = await supabase
      ...
      .or(
        `committed_at.is.null,committed_at.lt.${new Date(
          Date.now() - COOLOFF_HOURS * 3600 * 1000
        ).toISOString()}`
      )
    ...
    return new Response(JSON.stringify({ ok: true, mode: 'pull', count: targets.length }), {
      status: 202,
      headers: { 'content-type': 'application/json' },
    })
  } catch (error) {
